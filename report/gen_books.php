...
{	//echo $tb[0];
	$item=$row['itemnumber'];
	$query4="SELECT * FROM old_issues WHERE itemnumber='$item' AND returndate>='$from' AND returndate<='$to'";
	$run4=mysql_query($query4);
	//echo mysql_num_rows($run4);
	$row3=mysql_fetch_array($run4);
	$item1=$row3['itemnumber'];
	echo mysql_error();
	$query2="SELECT * FROM accountlines WHERE itemnumber='$item1' AND accounttype='F' AND date>='$from' AND date<='$to' ORDER BY timestamp ASC";
	...
	while($row1=mysql_fetch_array($run2))
	{
		...
		{	
			...
			$borrower=$row1['borrowernumber'];
			 ...
			 //changed from $query4="SELECT * FROM old_issues WHERE itemnumber='$item' AND borrowernumber='$borrower';
